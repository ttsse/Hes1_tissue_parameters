...
global alpha_d alpha_m alpha_p alpha_n mu_d mu_m mu_p mu_n D_d h gamma ;
alpha_d = 0.05;
alpha_m = 0.2;
alpha_p = 0.05;
alpha_n = 0.1;
...
if version == 1
    % numerically solve model for homogeneous initial conditions
    soln = pdepe(s,@tissue,@homtissueics,@tissuebcs,x,t);
    d = soln(:,:,1); % Dll1 solution
    p = soln(:,:,2); % Hes1 protein solution
    m = soln(:,:,3); % Hes1 mRNA solution
    n = soln(:,:,4); % Ngn2 solution

    % plot time behaviour of system at the last space point (all space points
    % have the same solution)
    plot(t,d(:,end), '-g', 'Linewidth', 4, 'Displayname', 'Dll1')
    hold on
    plot(t,p(:,end), '-b', 'Linewidth', 4, 'Displayname', 'Hes1 protein')
    hold on
    plot(t,m(:,end), '-r', 'Linewidth', 4, 'Displayname', 'Hes1 mRNA')
    hold on
    plot(t,n(:,end), '-k', 'Linewidth', 4, 'Displayname', 'Ngn2')
    hold off
    legend('Fontsize', 13)
...
elseif version == 2
    % numerically solve model for heterogeneous initial conditions
    soln = pdepe(s,@tissue,@hettissueics,@tissuebcs,x,t);
    d = soln(:,:,1); % Dll1 solution
    p = soln(:,:,2); % Hes1 protein solution
    m = soln(:,:,3); % Hes1 mRNA solution
    n = soln(:,:,4); % Ngn2 solution
    ...
    steps = 180;
    subplot(2,2,1);
    fig1 = pcolor(t(1:length(d)),x,transpose(p));
    set(fig1, 'EdgeColor', 'none');
    title('A', 'Fontsize', 16)
    xlabel('time (minutes)', 'Fontsize', 16)
    ylabel('x (\mum)', 'Fontsize', 16)
    ...
    subplot(2,2,2);
    fig2 = pcolor(t(1:length(d)),x,transpose(m));
    set(fig2, 'EdgeColor', 'none');
    title('B', 'Fontsize', 16)
    xlabel('time (minutes)', 'Fontsize', 16)
    ylabel('x (\mum)', 'Fontsize', 16)
    ...
    subplot(2,2,3);
    fig3 = pcolor(t(1:length(d)),x,transpose(d));
    set(fig3, 'EdgeColor', 'none');
    title('C', 'Fontsize', 16)
    xlabel('time (minutes)', 'Fontsize', 16)
    ylabel('x (\mum)', 'Fontsize', 16)
    ...
    subplot(2,2,4);
    fig4 = pcolor(t(1:length(d)),x,transpose(n));
    set(fig4, 'EdgeColor', 'none');
    title('D', 'Fontsize', 16)
    xlabel('time (minutes)', 'Fontsize', 16)
    ylabel('x (\mum)', 'Fontsize', 16)
    ...
    function [c,f,s] = tissue(x,t,u,DuDx)
    global D_d alpha_d alpha_m alpha_p alpha_n mu_d mu_m mu_p mu_n h gamma ;
    % u = [D, P, M, N]
    c = [1; 1; 1; 1];
    % model with or without diffusion (comment out the appropriate line to see relevant behaviour)
    f = [D_d; 0; 0; 0] .* DuDx;
%     f = [0; 0; 0; 0] .* DuDx;
    F1 = alpha_d * u(4) - mu_d * u(1);
    F2 = alpha_p * u(3) - mu_p * u(2);
    F3 = alpha_m * u(1) /(1 + u(2)^h) - mu_m * u(3);
    F4 = alpha_n / (1 + u(2)^gamma) - mu_n * u(4);
    s = [F1; F2; F3; F4];
    end
