...
iterations = [8; 189; 1428];
seq_pos_std = performance_summary_table.Mean(1:3)+performance_summary_table.StandardDeviation(1:3);
seq_neg_std = performance_summary_table.Mean(1:3)-performance_summary_table.StandardDeviation(1:3);
par_pos_std = performance_summary_table.Mean(4:6)+performance_summary_table.StandardDeviation(4:6);
par_neg_std = performance_summary_table.Mean(4:6)-performance_summary_table.StandardDeviation(4:6);
figure(1)
...
hold on
plot(iterations, performance_summary_table.Mean(1:3), 'r-', 'LineWidth', 2, 'DisplayName', 'sequential')
hold on
...
hold on
plot(iterations, performance_summary_table.Mean(4:6), 'Color', [0.196, 0.332, 0.482], 'LineStyle', '-',  'LineWidth', 2, 'DisplayName', 'parallel')
ax = gca;
ax.FontSize = 13;
